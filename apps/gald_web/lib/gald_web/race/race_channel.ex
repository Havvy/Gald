defmodule GaldWeb.RaceChannel do
  use Phoenix.Channel
  require Logger
  import Destructure
  alias GaldWeb.RaceManager
    ...
    if Regex.match?(~R/[^\p{L}0-9-]/, player_name) do
      ...
      case Gald.Race.new_player(race, player_name) do
        {:ok, {input, output}} ->
          ...
          GenEvent.add_handler(output, GaldWeb.PlayerOutToSocketHandler, d%{socket})
  ...
  def terminate({:shutdown, left_or_closed}, socket) do
    ...
    GaldWeb.RaceManager.delete_viewer(get_internal_name(socket), socket.transport_pid)
